...
export function parseNumberInput(normalizedParsingInput: string): ParseResult {
  ...
  console.log(
    `ns=${NumeralSystem[ns]}, unit=${Unit[unit]}, normalized decimal value=${unsignedNumericValue.toString()}`,
  );
  ...
  if (unsignedNumericValue.isNegative()) {
    throw new Error(
      'numeric value is negative! There must be an error in the code. We handle negative numbers via an extra flag! bignumber must be positive',
    );
  }
  if (wholePart.isNegative()) {
    throw new Error(
      'wholePart is negative! There must be an error in the code. We handle negative numbers via an extra flag! bignumber must be positive',
    );
  }
  if (fractionPart?.isNegative()) {
    throw new Error(
      'fractionPart is negative! There must be an error in the code. We handle negative numbers via an extra flag! bignumber must be positive',
    );
  }
